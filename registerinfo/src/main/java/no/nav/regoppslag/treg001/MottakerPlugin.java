...
public class MottakerPlugin extends JaxbHelper<Mottaker> implements ElementEnricherPlugin {
	...
	public Node processElement(Node content, Map<String, Object> valueMap, String tema) throws RegOppslagSecurityException {
		...
		SpraakKodeMapper spraakKodeMapper = (SpraakKodeMapper) valueMap.get(ValueMapKeys.MAALFORM.name());
		metrics.pluginReceived(MetricLabels.SERVICE_CODE_TREG001, PLUGIN_NAME);

		validateElementType(content);

		if (dokumenttypeId == null) {
			throw new RegoppslagIllegalArgumentException(format("Feil i %s, dokumentTypeId kan ikke være tom", PLUGIN_NAME), HttpStatus.BAD_REQUEST);
		}
	...
	private Mottaker getMottakerFraPersonV3(SpraakKodeMapper spraakKodeMapper, Mottaker mottaker, String dokumenttypeId) {
		...
		if (mottaker.isBerik()) {
			if (AktoerType.PERSON.equals(mottaker.getTypeKode())) {
				...
				Bruker person = personV3Consumer.hentPerson(mottaker.getId(), MetricLabels.SERVICE_CODE_TREG001);
				mottakerTo = personV3Mapper.map(person, MetricLabels.SERVICE_CODE_TREG001);
			} else {
				Organisasjon organisasjon = organisasjonV4Consumer.hentOrganisasjon(mottaker.getId());
				mottakerTo = organisasjonV4Mapper.map(mottaker.getId(), organisasjon, MetricLabels.SERVICE_CODE_TREG001);
			}
	...
	private void validateMottaker(Mottaker mottaker)  {
		...
		if (mottaker.getTypeKode() == null) {
			throw new RegoppslagIllegalArgumentException(format("Feil i %s: Mottakerdata mangler AktoerType. AktoerType kan ikke være null.", PLUGIN_NAME), HttpStatus.BAD_REQUEST);
		}

		if (StringUtils.isBlank(mottaker.getId()) || mottaker.getId().trim().isEmpty()) {
			throw new RegoppslagIllegalArgumentException(format("Feil i %s: Mottakerdata mangler mottakerId", PLUGIN_NAME), HttpStatus.BAD_REQUEST);
		}
