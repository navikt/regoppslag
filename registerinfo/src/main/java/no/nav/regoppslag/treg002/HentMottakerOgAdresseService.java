...
public class HentMottakerOgAdresseService {
	...
	public HentMottakerOgAdresseResponse hentMottakerOgAdresseInfo(HentMottakerOgAdresseRequest request) throws RegOppslagSecurityException {
		...
		try {
			...
			return AktoerType.PERSON.name().equals(request.getType()) ? hentMottakerOgAdresseForPerson(request) : hentMottakerOgAdresseForOrg(request);
	...
	private HentMottakerOgAdresseResponse hentMottakerOgAdresseForPerson(HentMottakerOgAdresseRequest request) {
		if (StringUtils.isBlank(request.getTema()) && AktoerType.PERSON.name().equals(request.getType())) {
			log.info("hentPersonV3 fra HentMottakerOgAdresseService"); //TODO: remove this log when is ready MMA-5754
			Bruker bruker = personV3Consumer.hentPerson(request.getIdentifikator(), MetricLabels.SERVICE_CODE_TREG002);
			MottakerTo mottakerTo = personV3Mapper.map(bruker, MetricLabels.SERVICE_CODE_TREG002);
		...
		PdlMottakerInfo pdlMottakerInfo = mapPDLResponse.mapHentPerson(pdlGraphQLConsumer.hentPerson(request.getIdentifikator(), request.getTema()), MetricLabels.SERVICE_CODE_TREG002, request.getTema());
		return HentMottakerOgAdresseResponse.builder()
				.identifikator(request.getIdentifikator())
	...
	private HentMottakerOgAdresseResponse hentMottakerOgAdresseForOrg(HentMottakerOgAdresseRequest request) {
		...
		MottakerTo mottakerTo = organisasjonV4Mapper.map(request.getIdentifikator(), organisasjon, MetricLabels.SERVICE_CODE_TREG002);
		return HentMottakerOgAdresseResponse.builder()
				.identifikator(request.getIdentifikator())
				.navn(mottakerTo.getMottaker().getNavn())
				.adresse(adresseMapper.map(mottakerTo.getMottaker()))
	...
	private void validateInput(HentMottakerOgAdresseRequest request) {
		...
		if (request == null) {
			throw new RegoppslagIllegalArgumentException("Request body er tom. " + UGYLDIG_INPUT, HttpStatus.BAD_REQUEST);
		}

		if (request.getIdentifikator() == null) {
			throw new RegoppslagIllegalArgumentException("Identifikator kan ikke være null. " + UGYLDIG_INPUT, HttpStatus.BAD_REQUEST);
		}

		if (request.getType() == null) {
			throw new RegoppslagIllegalArgumentException("Mottakertype kan ikke være null. " + UGYLDIG_INPUT, HttpStatus.BAD_REQUEST);
		} else if (!(AktoerType.PERSON.name().equals(request.getType()) || AktoerType.ORGANISASJON.name()
				.equals(request.getType()))) {
			throw new RegoppslagIllegalArgumentException(String.format("Mottakertype var %s. Det må være PERSON eller ORGANISASJON.", request
					.getType()) + UGYLDIG_INPUT, HttpStatus.BAD_REQUEST);
		}
	}
