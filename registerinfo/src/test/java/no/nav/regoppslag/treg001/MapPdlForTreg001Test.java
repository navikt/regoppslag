...
class MapPdlForTreg001Test {
	...
	public void shouldMapTreg001MottakerAdresseFraPdl() {
		when(pdlGraphQLConsumer.hentPerson(PDLResponseUtil.PERSON_IDENT, TEMA)).thenReturn(PDLResponseUtil.createPdlHentPersonWithBostedsadresse());
		...
		Assertions.assertEquals(PDLResponseUtil.PERSON_IDENT, mottaker.getId());
	...
	public void shouldMapUtenlandiskAdresseFraPdl() {
		when(pdlGraphQLConsumer.hentPerson(PDLResponseUtil.PERSON_IDENT, TEMA)).thenReturn(PDLResponseUtil.createPdlHentPersonUtenlandskAdresse());
		Mottaker mottaker = pdlForTreg001.getMottakerFraPdl(TEMA, createMottaker());
		...
		Assertions.assertEquals(PDLResponseUtil.SWEDEN_UTENLANDSK, adresse.getLand());
	}

	private Mottaker createMottaker() {
		Mottaker mottaker = new Person();
		mottaker.setId(PDLResponseUtil.PERSON_IDENT);
