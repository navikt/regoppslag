 ...
 */
@Ignore("Finner ikke fila det refereres til")
public class ValiderOgKompletterBrevdataCompTest {
	...
	ValiderOgKompletterBrevdataResponse response = new ValiderOgKompletterBrevdataResponse("<ole>brumm</ole>");
	KompletterBrevdataService kompletterBrevdataService = mock(KompletterBrevdataService.class);  //TODO fjern midlertidig mock av service. Bytt ut med wiremock av endepunktene som plugin kjører mot
	HentMottakerOgAdresseService hentMottakerOgAdresseService = mock(HentMottakerOgAdresseService.class);  //TODO fjern midlertidig mock av service. Bytt ut med wiremock av endepunktene som plugin kjører mot
	...
	public void shouldKomplettereBrevdata() throws Exception {
		when(kompletterBrevdataService.hentBrevdataFraRegistre(any())).thenReturn(response);
	...
	public void shouldThrowFunctionalException() throws Exception {
		...
		when(kompletterBrevdataService.hentBrevdataFraRegistre(any())).thenThrow(new RegOppslagFunctionalException(feilmelding));
		mvc.perform(post(KOMPLETTER_BREVDATA_URI_PATH).contentType(MediaType.APPLICATION_JSON)
				.content(request))
	...
	public void shouldThrowTechnicalException() throws Exception {
		String feilmelding = "feilmelding";
		when(kompletterBrevdataService.hentBrevdataFraRegistre(any())).thenThrow(new RegOppslagTechnicalException(feilmelding));
