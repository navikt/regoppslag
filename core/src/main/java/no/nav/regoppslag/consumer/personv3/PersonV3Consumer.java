...
public class PersonV3Consumer {
	...
	public Bruker hentPerson(final String personidentifikator, String serviceCode) throws RegOppslagSecurityException {
		...
		try {
			...
			if(person != null && person.getDiskresjonskode() != null && !StringUtils.isBlank(person.getDiskresjonskode().getValue())) {
			...
			throw new RegOppslagIkkeFunnetException(String.format("PersonV3.hentPerson fant ikke person, message=%s",hentPersonPersonIkkeFunnet
					.getMessage()), hentPersonPersonIkkeFunnet, PERSON_IKKE_FUNNET, HttpStatus.NOT_FOUND);
			...
			if (e.getCause() instanceof SamlTokenInterceptorException){
				throw new RegOppslagIkkeFunnetException(e.getMessage(), e, "PersonV3 - Mangler/Feil SAML token", HttpStatus.NOT_FOUND);
			}
