...
public class PersonV3Mapper {
	...
	public MottakerTo map(Bruker person, String serviceCode)  {
		...
		if (LAND_NORGE.equals(postadresse.getLand()) || StringUtils.isBlank(postadresse.getLand())) {
	...
	private void validateAdresse(Bruker person, Postadresse postadresse, String serviceCode) {
		...
		if (person.getGjeldendePostadressetype() != null && UKJENT_ADRESSE.equals(person.getGjeldendePostadressetype()
			...
			if (isPersonDod(person)) {
				throw new UkjentAdressePersonErDoed(serviceCode + " Mottaker er registrert som død og har gjeldendePostadressetype=UKJENT_ADRESSE", HttpStatus.GONE);
			}
			throw new UkjentAdresseException(serviceCode + " Kunne ikke mappe postadresse for mottaker fordi gjeldendePostadressetype=UKJENT_ADRESSE", HttpStatus.NOT_FOUND);
		}

		if (isBlankPostadresse(postadresse)) {
			if (isPersonDod(person)) {
				throw new UkjentAdressePersonErDoed("Mottaker er registrert som død og har ugyldig postadresse. Adresse mangler adresselinje1, postnummer, poststed og land.", HttpStatus.GONE);
			}
	...
	private boolean isBlankPostadresse(Postadresse postadresse) {
		return StringUtils.isBlank(postadresse.getAdresselinje1()) && StringUtils.isBlank(postadresse.getLand()) && StringUtils.isBlank(postadresse.getPostnummer()) && StringUtils.isBlank(postadresse
				.getPoststed());
	}
